...
class FilmConditioning(nn.Module):
    ...
    def __call__(self, conv_filters: jnp.ndarray, conditioning: jnp.ndarray):
        ...
        """
        assert conditioning is not None, "FiLM conditioning requires input."
...
class Model(nn.Module):
  ...
  @nn.compact
  def __call__(self, x, *, train=False, film_conditioning=None):
    if self.raw_input:
      x = x / 255.0
    ...
    if self.use_film_conditioning:
      x = FilmConditioning()(x, film_conditioning)
