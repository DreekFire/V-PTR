...
from vptr.utils.types import DataType
...
class MixingReplayBuffer():
    ...
    def __init__(
        ...
        self.streaming_iter = 1

        self.streaming_traj_count = [0, 0]
    ...
    # this is for streaming the online data
    # suppose you have 800 trajectories in total, with max_streaming_iter=10 and variant.streaming_interval = 100,
    # then you will stream 800 // 10 = 80 trajs for each 100 steps
    def add_streaming_data(self, max_streaming_iter=10):
        for b in self.replay_buffers:
            streaming_traj_count = b._traj_counter // max_streaming_iter * self.streaming_iter
            streaming_traj_count = min(streaming_traj_count, b._traj_counter)
            start, end = b.traj_bounds[streaming_traj_count - 1]
            b.streaming_buffer_size = end
            print("add_streaming_data", streaming_traj_count, end)
        self.streaming_iter += 1

    def add_streaming_data_1traj(self, pos_neg_weight=7):
        buffer_id = random.choices([0, 1], weights=(1, pos_neg_weight), k=1)[0]
        b = self.replay_buffers[0]
        streaming_traj_count = self.streaming_traj_count[buffer_id] + 1
        streaming_traj_count = min(streaming_traj_count, b._traj_counter)
        start, end = b.traj_bounds[streaming_traj_count - 1]
        b.streaming_buffer_size = end
        print("buffer", buffer_id)
        print("add_streaming_data", buffer_id, streaming_traj_count, end)
        self.streaming_traj_count[buffer_id] += 1
