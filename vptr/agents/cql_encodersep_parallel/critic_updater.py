...
from vptr.utils.types import DatasetDict
...
def update_critic(
        ...
        reward_regression: bool = False, mc_regression: bool = False, action_jitter_scale: float = 0.1,
    ):
    ...
    if max_q_backup:
        next_actions, next_log_probs = dist.sample_and_log_prob(seed=key, sample_shape=(NUM_CQL_REPEAT,))
        next_actions = jnp.swapaxes(next_actions, 0, 1).reshape(-1, *next_actions.shape[2:])
        next_log_probs = jnp.swapaxes(next_log_probs, 0, 1).reshape(-1, *next_log_probs.shape[2:])
    else:
        next_actions, next_log_probs = dist.sample_and_log_prob(seed=key, sample_shape=())

    if use_basis_projection:
        bp_next_actions, _ = dist.sample_and_log_prob(seed=bp_key)
    ...
    def critic_loss_fn(critic_encoder_params: Params, critic_decoder_params: Params):
        ...
        if action_jitter_scale > 0:
            jitter = jax.random.normal(key_q5, batch['actions'].shape) * action_jitter_scale
            noised_actions = batch['actions'] + jitter
            noised_actions = jnp.clip(noised_actions, -0.99, 0.99)

        data_args = [embed_curr_obs, batch['actions']]
        if action_jitter_scale > 0:
            noised_data_args = embed_curr_obs, noised_actions
        ...
        if hasattr(critic_decoder, 'batch_stats') and critic_decoder.batch_stats is not None:
            qs, new_model_state_decoder = critic_decoder.apply_fn(
                ...
                *data_args, mutable=['batch_stats'],
                rngs={'dropout': key_q2}, training=True)
            
            if action_jitter_scale > 0:
                qs_noised, new_model_state_decoder = critic_decoder.apply_fn(
                    {'params': critic_decoder_params, 'batch_stats': critic_decoder.batch_stats},
                    *noised_data_args, mutable=['batch_stats'],
                    rngs={'dropout': key_q2}, training=False)
        else:
            qs = critic_decoder.apply_fn(
                {'params': critic_decoder_params}, *data_args,
                rngs={'dropout': key_q2})
            
            if action_jitter_scale > 0:
                qs_noised = critic_decoder.apply_fn(
                    {'params': critic_decoder_params}, *noised_data_args,
                    rngs={'dropout': key_q2})
        ...
        qpi_args = [embed_curr_obs_tiled, policy_actions]
        if hasattr(critic_decoder, 'batch_stats') and critic_decoder.batch_stats is not None:
            q_pi, _ = critic_decoder.apply_fn(
                {'params': critic_decoder_params, 'batch_stats': critic_decoder.batch_stats},
                *qpi_args, mutable=['batch_stats'], training=True,
                rngs={'dropout': key_q3})
        else:
            q_pi = critic_decoder.apply_fn(
                {'params': critic_decoder_params}, *qpi_args,
                rngs={'dropout': key_q3})
            
        # check if q(s, pi(s)) exceeds q(s, a)
        q_over = q_pi.reshape(q_pi.shape[0], -1, NUM_CQL_REPEAT) > jnp.expand_dims(qs, axis=2)
        ...
        q_random_args = [embed_curr_obs_tiled, random_actions]
        if hasattr(critic_decoder, 'batch_stats') and critic_decoder.batch_stats is not None:
            q_random, _ = critic_decoder.apply_fn(
                {'params': critic_decoder_params, 'batch_stats': critic_decoder.batch_stats},
                *q_random_args, mutable=['batch_stats'], training=True,
                rngs={'dropout': key_q4})
        else:
            q_random = critic_decoder.apply_fn(
                {'params': critic_decoder_params}, *q_random_args,
                rngs={'dropout': key_q4})
        ...
        cql_loss_per_element = lse_q - qs_noised
        ...
        if reward_regression:
            critic_loss = ((qs - batch['rewards'])**2).mean()
        elif mc_regression:
